    ...
    Contour.connectors.ConnectorBase = Contour.Base.extend({
        ...
        *       })
        ...
        data: function (measures, extras) {
            ...
            var _this = this;
            var filteredData = this._filterSelector ? _.filter(_this._data, function (row, index) {
                return _this._filterSelector.call(_this, row, index, _this._headers);
            }) : this._data;
            var rolledRight = this._newMeasure ? _.map(filteredData, function (row, index) {
                var r = row.slice();
                r.push(_this._newMeasure.fn.call(_this, row, index, _this._headers));
                return r;
            }) : filteredData;
            ...
            var result = _.map(measures, function (m) {
                return _this._generateSeries.call(_this, m, rolledUp, extras);
            });
        ...
        _generateSeries: function (measure, data, extras) {
            ...
            var result = _.map(data, function (d) {
                ...
                if (extras) {
                    var indices = _.map(extras, function (f) {
                        return _this._getMeasureIndex(f);
                    });
                    var extrasObj = {};
    ...
    *       })
    ...
    Contour.connectors.Csv = Contour.connectors.ConnectorBase.extend({
        ...
        parse: function (raw, headerRow) {
            ...
            var rows = raw.split(/\r\n|\r|\n/);
            this._headers = headerRow ? _.map(rows.shift().split(this.splitPatter), function (d) {
                return d.toLowerCase().trim();
            }) : _.range(0, rows[0].length);
