...
$data.InMemoryConverter = {
    fromDb: {
        ...
        '$data.Guid': function (guid) {
            return guid ? $data.parseGuid(guid).toString() : guid;
        },
    ...
    escape: {
        ...
        '$data.Blob': function (blob) {
            return "'" + $data.Blob.toString(blob) + "'";
        },
        '$data.Object': function (o) {
...
$data.mongoDBConverter = {
    fromDb: {
        ...
        '$data.Time': function (date) {
            return date ? Container.convertTo(date, $data.Time) : date;
        },
        '$data.String': $data.Container.proxyConverter,
        '$data.Boolean': $data.Container.proxyConverter,
        '$data.Blob': function (v) {
            return v ? $data.Container.convertTo(typeof v === 'string' ? atob(v) : v.buffer || v, $data.Blob) : v;
        },
        '$data.Object': function (o) {
            if (o === undefined) {
                return new $data.Object();
    ...
    VisitPagingExpression: function (expression, context) {
        ...
        switch (expression.nodeType) {
            case $data.Expressions.ExpressionType.Skip:
                context.options.skip = pagingContext.data;
                break;
            case $data.Expressions.ExpressionType.Take:
                context.options.limit = pagingContext.data;
                break;
            default:
...
$C('$data.storageProviders.mongoDB.mongoDBCompiler', $data.Expressions.EntityExpressionVisitor, null, {
    ...
    compile: function (query) {
        ...
        var modelBinder = new $data.modelBinder.mongoDBModelBinderConfigCompiler(query, this.includes.filter(function (it) {
            return it.mapped;
        }), true);
...
$C('$data.storageProviders.mongoDB.mongoDBProvider', $data.StorageProviderBase, null,
    {
        constructor: function (cfg, ctx) {
            ...
            if (this.providerConfiguration.server) {
                if (typeof this.providerConfiguration.server === 'string') this.providerConfiguration.server = [
                    { address: this.providerConfiguration.server.split(':')[0] || '127.0.0.1', port: this.providerConfiguration.server.split(':')[1] || 27017 }
                ];
                        ...
                        if (self.context._entitySetReferences.hasOwnProperty(i)) {
                            client.collectionNames({ namesOnly: true }, function (error, names) {
                                names = names.map(function (it) {
                                    return it.slice(it.lastIndexOf('.') + 1);
                                });
...
$data.Class.define('$data.storageProviders.mongoDB.mongoDBProvider.ClientObjectID', null, null, {
    constructor: function () {
        ...
        this.toString = this.toLocaleString = this.valueOf = function () {
            return btoa(time + machine + pid + inc);
        };
    },
    value: { value: null }
}, {
