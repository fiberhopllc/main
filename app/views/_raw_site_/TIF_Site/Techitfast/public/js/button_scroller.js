$(function() {
	...
	if(startingIndex>-1){prevButton=startingIndex;}
	if(startingIndex<=-1){prevButton=0;startingIndex=0;}
	if(startingIndex>pLength-1){prevButton=pLength-1;startingIndex=pLength-1;}
	if(useAsLastIndex && !placeholder){placeholder=true;startingIndex++;}
	for(var i=placeholder?startingIndex:pLength;i<bLength;i++){$('.window_control#b'+i).remove();}
	...
	$(".window_control").click(function() {	
		...
		if(curButton != prevButton){
			...
			if(!animating){
				...
				nextItem.show("slide",{direction:moveRight?"left":"right"},animationTime,function(){animating=false});
			}
		}			
		return false;
	});
