...
'*	void				RemoveLast(Object value)
'*	void				RemoveAll(Object p_value)
'		void 				RemoveAt(int index)
'		int 				IndexOf(Object value)
'*	int					IndexOfLast(Object value
'*	array				IndicesOf(Object value)
'		bool 				Contains(Object value)
'*	bool				ContainsMultiple(Object value)
'		void 				Clear()
...
class Collection
	...
	'**
	'* Removes the last instance of the specified item from the collection.
	'* @param p_value (ref object) - The object to remove from the collection.
	'**
	public function RemoveLast(ByRef p_value)
			call me.RemoveAt(IndexOfLast(p_value))
	end function
	
	'**
	'* Removes all instances of the specified item from the collection.
	'* @param p_value (ref object) - The object reference used to remove all instances from the collection.
	'**
	public function RemoveAll(ByRef p_value)
		allIndices = IndicesOf(p_value)
		for i=0 to ubound(allIndices)
			call me.RemoveAt(allIndices(i)-i)
		next
	end function
	...
	'* @returns (int) The index of the item in the collection, or -1 if it was not found.
	'**
	public function IndexOfLast(ByRef p_value)
		IndexOfLast = -1
		for i = 0 to m_size
			if m_data(m_size-i) = p_value then
				IndexOfLast = m_size-i
				exit for
			end if
		next
	end function
	...
	'**
	'* Gets all indices of the specified item in the collection if it exists in the collection.
	'* @param p_value (ref object) - The item to find in the collection.
	'* @returns (array) All indices of the item in the collection.
	'**
	public function IndicesOf(ByRef p_value)
		Indices = array()
		for i=0 to m_size
			if m_data(i) = p_value then
				iIndex = ubound(Indices)+1
				Redim Preserve Indices(iIndex)
				Indices(iIndex) = i
			end if
		next
		IndicesOf = Indices
	...
	'* @returns (bool) TRUE if the item is in the collection, otherwise, FALSE.
	'**
	public function ContainsMultiple(ByRef p_value)
		ContainsMultiple = (ubound(me.IndicesOf(p_value)) > 0)
	end function
