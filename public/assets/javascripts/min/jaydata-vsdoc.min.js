...
function wrapCode(fn){var t=this;function r(){fn.apply(t,arguments);fn=function(){}}
return r;}
...
return pHandler.getPromise();},toArray:function(onResult_items,transaction){if(onResult_items instanceof $data.Array){return this.toArray(function(results){onResult_items.length=0;results.forEach(function(item,idx){onResult_items.push(item);});});}
...
var typeShortName=typeData.Name;var containerName="";if(typeData.Name.lastIndexOf('.')>0){containerName=typeData.Name.substring(0,typeData.Name.lastIndexOf('.'));typeShortName=typeData.Name.substring(typeData.Name.lastIndexOf('.')+1);}
var conainers=doc.querySelectorAll("EntityContainer[Name = '"+containerName+"']");for(var j=0;j<conainers.length;j++){var entitySetDef=conainers[j].querySelector("EntitySet[Name = '"+typeShortName+"']");if(entitySetDef!=null){typeData.Name=entitySetDef.attributes["EntityType"].value;break;}}
...
var complex=doc.querySelectorAll("ComplexType");for(var i=0;i<complex.length;i++){var cns=complex[i].parentNode.attributes["Namespace"].value;var data=!cns?complex[i].attributes["Name"].value:(cns+"."+complex[i].attributes["Name"].value);discoveredData.push({Name:data,Fields:[]});}
for(var i=0;i<discoveredData.length;i++){var row=discoveredData[i];if(row.Fields.length>0){result+=row.Name+":"+row.Fields.join(",")+";";}
else{result+=row.Name+";";}}}
return result;},_discoverTypeDependencies:function(types,doc,withNavPropertis,withKeys){var allowedTypes=[];var allowedTypeNames=[];var collect=[];for(var i=0;i<types.length;i++){var idx=collect.indexOf(types[i].Name);if(idx>=0){collect.splice(idx,1);}
this._discoverType(types[i],doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys,true,collect);}
for(var i=0;i<collect.length;i++){this._discoverType({Name:collect[i],Fields:[]},doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys,false,[]);}
return allowedTypes;},_discoverType:function(typeData,doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys,collectTypes,collectedTypes){var typeName=typeData.Name;if(allowedTypeNames.indexOf(typeName)>=0){return;}
console.log("Discover: "+typeName);var typeShortName=typeName;var typeNamespace='';if(typeName.lastIndexOf('.')>0){typeNamespace=typeName.substring(0,typeName.lastIndexOf('.'));typeShortName=typeName.substring(typeName.lastIndexOf('.')+1);}
var schemaNode=doc.querySelector("Schema[Namespace = '"+typeNamespace+"']");if(schemaNode!=null){var typeNode=schemaNode.querySelector("EntityType[Name = '"+typeShortName+"'], ComplexType[Name = '"+typeShortName+"']");if(typeNode!=null){allowedTypes.push(typeData);allowedTypeNames.push(typeName);if(withKeys&&typeData.Fields.length>0){var keys=typeNode.querySelectorAll("Key PropertyRef");if(keys!=null){for(var j=0;j<keys.length;j++){var keyField=keys[j].attributes["Name"].value;if(typeData.Fields.indexOf(keyField)<0)
typeData.Fields.splice(j,0,keyField);}}}
if(withNavPropertis){var navPropNodes=typeNode.querySelectorAll("NavigationProperty");for(var j=0;j<navPropNodes.length;j++){var navProp=navPropNodes[j];if(typeData.Fields.length==0||typeData.Fields.indexOf(navProp.attributes["Name"].value)>=0){var FromRole=navProp.attributes["FromRole"].value;var ToRole=navProp.attributes["ToRole"].value;var association=schemaNode.querySelector("Association End[Role = '"+FromRole+"']:not([Type = '"+typeName+"'])");if(association==null){association=schemaNode.querySelector("Association End[Role = '"+ToRole+"']:not([Type = '"+typeName+"'])");}
if(association!=null){var nav_type=association.attributes["Type"].value;if(collectTypes){if(collectedTypes.indexOf(nav_type)<0&&allowedTypeNames.indexOf(nav_type)<0)
collectedTypes.push(nav_type);}
else{this._discoverType({Name:nav_type,Fields:[]},doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys,false,collectedTypes);}}}}}}}},_discoverProperyDependencies:function(types,doc,withNavPropertis,withKeys){var allowedTypes=[];var allowedTypeNames=types.map(function(t){return t.Name;});for(var i=0;i<types.length;i++){this._discoverProperties(types[i],doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys);}
return allowedTypes;},_discoverProperties:function(typeData,doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys){var typeName=typeData.Name;console.log("Discover: "+typeName);var hasProperty=typeData.Fields.length!=0;var typeShortName=typeName;var typeNamespace='';if(typeName.lastIndexOf('.')>0){typeNamespace=typeName.substring(0,typeName.lastIndexOf('.'));typeShortName=typeName.substring(typeName.lastIndexOf('.')+1);}
...
if(withNavPropertis){var navPropNodes=typeNode.querySelectorAll("NavigationProperty");for(var j=0;j<navPropNodes.length;j++){var navProp=navPropNodes[j];var nav_name=navProp.attributes["Name"].value;var types=[navProp.attributes["FromRole"].value,navProp.attributes["ToRole"].value];var nav_type='';for(var t=0;t<types.length;t++){var association=schemaNode.querySelector("Association End[Role = '"+types[t]+"']");if(association!=null){nav_type=association.attributes["Type"].value;if(nav_type!=typeName||t==1)
break;}}
if(allowedTypeNames.indexOf(nav_type)>=0){typeData.Fields.push(nav_name);}}}}
else if(withKeys){var keys=typeNode.querySelectorAll("Key PropertyRef");if(keys!=null){for(var j=0;j<keys.length;j++){var keyField=keys[j].attributes["Name"].value;if(typeData.Fields.indexOf(keyField)<0)
