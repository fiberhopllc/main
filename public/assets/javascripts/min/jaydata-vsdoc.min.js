...
(function(window){if(typeof window.intellisense==='undefined'){window.intellisense={annotate:function(){},logMessage:function(){}};}})(window);(function(mod){if(typeof exports=="object"&&typeof module=="object")return mod(exports);if(typeof define=="function"&&define.amd)return define(["exports"],mod);mod(self.acorn||(self.acorn={}));})(function(exports){"use strict";exports.version="0.1.01";var options,input,inputLen,sourceFile;exports.parse=function(inpt,opts){input=String(inpt);inputLen=input.length;setOptions(opts);initTokenState();return parseTopLevel(options.program);};var defaultOptions=exports.defaultOptions={ecmaVersion:5,strictSemicolons:false,allowTrailingCommas:true,forbidReserved:false,locations:false,onComment:null,ranges:false,program:null,sourceFile:null};function setOptions(opts){options=opts||{};for(var opt in defaultOptions)if(!options.hasOwnProperty(opt))
...
return{line:line,column:offset-cur};};exports.tokenize=function(inpt,opts){input=String(inpt);inputLen=input.length;setOptions(opts);initTokenState();var t={};function getToken(forceRegexp){readToken(forceRegexp);t.start=tokStart;t.end=tokEnd;t.startLoc=tokStartLoc;t.endLoc=tokEndLoc;t.type=tokType;t.value=tokVal;return t;}
getToken.jumpTo=function(pos,reAllowed){tokPos=pos;if(options.locations){tokCurLine=tokLineStart=lineBreak.lastIndex=0;var match;while((match=lineBreak.exec(input))&&match.index<pos){++tokCurLine;tokLineStart=match.index+match[0].length;}}
var ch=input.charAt(pos-1);tokRegexpAllowed=reAllowed;skipSpace();};return getToken;};var tokPos;var tokStart,tokEnd;var tokStartLoc,tokEndLoc;var tokType,tokVal;var tokRegexpAllowed;var tokCurLine,tokLineStart;var lastStart,lastEnd,lastEndLoc;var inFunction,labels,strict;function raise(pos,message){var loc=getLineInfo(input,pos);message+=" ("+loc.line+":"+loc.column+")";var err=new SyntaxError(message);err.pos=pos;err.loc=loc;err.raisedAt=tokPos;throw err;}
var _num={type:"num"},_regexp={type:"regexp"},_string={type:"string"};var _name={type:"name"},_eof={type:"eof"};var _break={keyword:"break"},_case={keyword:"case",beforeExpr:true},_catch={keyword:"catch"};var _continue={keyword:"continue"},_debugger={keyword:"debugger"},_default={keyword:"default"};var _do={keyword:"do",isLoop:true},_else={keyword:"else",beforeExpr:true};var _finally={keyword:"finally"},_for={keyword:"for",isLoop:true},_function={keyword:"function"};var _if={keyword:"if"},_return={keyword:"return",beforeExpr:true},_switch={keyword:"switch"};var _throw={keyword:"throw",beforeExpr:true},_try={keyword:"try"},_var={keyword:"var"};var _while={keyword:"while",isLoop:true},_with={keyword:"with"},_new={keyword:"new",beforeExpr:true};var _this={keyword:"this"};var _null={keyword:"null",atomValue:null},_true={keyword:"true",atomValue:true};var _false={keyword:"false",atomValue:false};var _in={keyword:"in",binop:7,beforeExpr:true};var keywordTypes={"break":_break,"case":_case,"catch":_catch,"continue":_continue,"debugger":_debugger,"default":_default,"do":_do,"else":_else,"finally":_finally,"for":_for,"function":_function,"if":_if,"return":_return,"switch":_switch,"throw":_throw,"try":_try,"var":_var,"while":_while,"with":_with,"null":_null,"true":_true,"false":_false,"new":_new,"in":_in,"instanceof":{keyword:"instanceof",binop:7,beforeExpr:true},"this":_this,"typeof":{keyword:"typeof",prefix:true,beforeExpr:true},"void":{keyword:"void",prefix:true,beforeExpr:true},"delete":{keyword:"delete",prefix:true,beforeExpr:true}};var _bracketL={type:"[",beforeExpr:true},_bracketR={type:"]"},_braceL={type:"{",beforeExpr:true};var _braceR={type:"}"},_parenL={type:"(",beforeExpr:true},_parenR={type:")"};var _comma={type:",",beforeExpr:true},_semi={type:";",beforeExpr:true};var _colon={type:":",beforeExpr:true},_dot={type:"."},_question={type:"?",beforeExpr:true};var _slash={binop:10,beforeExpr:true},_eq={isAssign:true,beforeExpr:true};var _assign={isAssign:true,beforeExpr:true},_plusmin={binop:9,prefix:true,beforeExpr:true};var _incdec={postfix:true,prefix:true,isUpdate:true},_prefix={prefix:true,beforeExpr:true};var _bin1={binop:1,beforeExpr:true},_bin2={binop:2,beforeExpr:true};var _bin3={binop:3,beforeExpr:true},_bin4={binop:4,beforeExpr:true};var _bin5={binop:5,beforeExpr:true},_bin6={binop:6,beforeExpr:true};var _bin7={binop:7,beforeExpr:true},_bin8={binop:8,beforeExpr:true};var _bin10={binop:10,beforeExpr:true};exports.tokTypes={bracketL:_bracketL,bracketR:_bracketR,braceL:_braceL,braceR:_braceR,parenL:_parenL,parenR:_parenR,comma:_comma,semi:_semi,colon:_colon,dot:_dot,question:_question,slash:_slash,eq:_eq,name:_name,eof:_eof,num:_num,regexp:_regexp,string:_string};for(var kw in keywordTypes)exports.tokTypes[kw]=keywordTypes[kw];function makePredicate(words){words=words.split(" ");var f="",cats=[];out:for(var i=0;i<words.length;++i){for(var j=0;j<cats.length;++j)
...
function skipLineComment(){var start=tokPos;var startLoc=options.onComment&&options.locations&&new line_loc_t;var ch=input.charCodeAt(tokPos+=2);while(tokPos<inputLen&&ch!==10&&ch!==13&&ch!==8232&&ch!==8329){++tokPos;ch=input.charCodeAt(tokPos);}
if(options.onComment)
...
if(options.locations){++tokCurLine;tokLineStart=tokPos;}}else if(ch===10){++tokPos;++tokCurLine;tokLineStart=tokPos;}else if(ch<14&&ch>8){++tokPos;}else if(ch===47){var next=input.charCodeAt(tokPos+1);if(next===42){skipBlockComment();}else if(next===47){skipLineComment();}else break;}else if((ch<14&&ch>8)||ch===32||ch===160){++tokPos;}else if(ch>=5760&&nonASCIIwhitespace.test(String.fromCharCode(ch))){++tokPos;}else{break;}}}
...
function readToken_mult_modulo(){var next=input.charCodeAt(tokPos+1);if(next===61)return finishOp(_assign,2);return finishOp(_bin10,1);}
function readToken_pipe_amp(code){var next=input.charCodeAt(tokPos+1);if(next===code)return finishOp(code===124?_bin1:_bin2,2);if(next===61)return finishOp(_assign,2);return finishOp(code===124?_bin3:_bin5,1);}
function readToken_caret(){var next=input.charCodeAt(tokPos+1);if(next===61)return finishOp(_assign,2);return finishOp(_bin4,1);}
function readToken_plus_min(code){var next=input.charCodeAt(tokPos+1);if(next===code)return finishOp(_incdec,2);if(next===61)return finishOp(_assign,2);return finishOp(_plusmin,1);}
function readToken_lt_gt(code){var next=input.charCodeAt(tokPos+1);var size=1;if(next===code){size=code===62&&input.charCodeAt(tokPos+2)===62?3:2;if(input.charCodeAt(tokPos+size)===61)return finishOp(_assign,size+1);return finishOp(_bin8,size);}
if(next===61)
size=input.charCodeAt(tokPos+2)===61?3:2;return finishOp(_bin7,size);}
function readToken_eq_excl(code){var next=input.charCodeAt(tokPos+1);if(next===61)return finishOp(_bin6,input.charCodeAt(tokPos+2)===61?3:2);return finishOp(code===61?_eq:_prefix,1);}
...
var content=input.slice(start,tokPos);++tokPos;var mods=readWord1();if(mods&&!/^[gmsiy]*$/.test(mods))raise(start,"Invalid regexp flag");return finishToken(_regexp,new RegExp(content,mods));}
...
var next=input.charCodeAt(tokPos);if(next===69||next===101){next=input.charCodeAt(++tokPos);if(next===43||next===45)++tokPos;if(readInt(10)===null)raise(start,"Invalid number")
isFloat=true;}
...
break;default:out+=String.fromCharCode(ch);break;}}}else{if(ch===13||ch===10||ch===8232||ch===8329)raise(tokStart,"Unterminated string constant");out+=String.fromCharCode(ch);++tokPos;}}}
...
function setStrict(strct){strict=strct;tokPos=lastEnd;while(tokPos<tokLineStart){tokLineStart=input.lastIndexOf("\n",tokLineStart-2)+1;--tokCurLine;}
skipSpace();readToken();}
...
node.loc=new node_loc_t();if(options.ranges)
...
var loopLabel={kind:"loop"},switchLabel={kind:"switch"};function parseStatement(){if(tokType===_slash)
...
function parseBlock(allowStrict){var node=startNode(),first=true,strict=false,oldStrict;node.body=[];expect(_braceL);while(!eat(_braceR)){var stmt=parseStatement();node.body.push(stmt);if(first&&isUseStrict(stmt)){oldStrict=strict;setStrict(strict=true);}
first=false}
...
return expr;}
function parseExprOps(noIn){return parseExprOp(parseMaybeUnary(noIn),-1,noIn);}
function parseExprOp(left,minPrec,noIn){var prec=tokType.binop;if(prec!=null&&(!noIn||tokType!==_in)){if(prec>minPrec){var node=startNodeFrom(left);node.left=left;node.operator=tokVal;next();node.right=parseExprOp(parseMaybeUnary(noIn),prec,noIn);var node=finishNode(node,/&&|\|\|/.test(node.operator)?"LogicalExpression":"BinaryExpression");return parseExprOp(node,minPrec,noIn);}}
return left;}
function parseMaybeUnary(noIn){if(tokType.prefix){var node=startNode(),update=tokType.isUpdate;node.operator=tokVal;node.prefix=true;next();node.argument=parseMaybeUnary(noIn);if(update)checkLVal(node.argument);else if(strict&&node.operator==="delete"&&node.argument.type==="Identifier")
...
function parseExprAtom(){switch(tokType){case _this:var node=startNode();next();return finishNode(node,"ThisExpression");case _name:return parseIdent();case _num:case _string:case _regexp:var node=startNode();node.value=tokVal;node.raw=input.slice(tokStart,tokEnd);next();return finishNode(node,"Literal");case _null:case _true:case _false:var node=startNode();node.value=tokType.atomValue;node.raw=tokType.keyword
next();return finishNode(node,"Literal");case _parenL:var tokStartLoc1=tokStartLoc,tokStart1=tokStart;next();var val=parseExpression();val.start=tokStart1;val.end=tokEnd;if(options.locations){val.loc.start=tokStartLoc1;val.loc.end=tokEndLoc;}
if(options.ranges)
...
function parseNew(){var node=startNode();next();node.callee=parseSubscripts(parseExprAtom(),true);if(eat(_parenL))node.arguments=parseExprList(_parenR,false);else node.arguments=[];return finishNode(node,"NewExpression");}
...
return elts;}
function parseIdent(liberal){var node=startNode();node.name=tokType===_name?tokVal:(liberal&&!options.forbidReserved&&tokType.keyword)||unexpected();next();return finishNode(node,"Identifier");}});(function(global){if(typeof window==="undefined"){window=this;}
...
if(!console.warn)console.warn=function(){};if(!console.error)console.error=function(){};(function($data){$data.__namespace=true;$data.version="JayData 1.3.6";$data.versionNumber="1.3.6";$data.root={};$data.Acorn=$data.Acorn||(typeof acorn=='object'?acorn:undefined);$data.Esprima=$data.Esprima||(typeof esprima=='object'?esprima:undefined);})($data);Exception=function(message,name,data){Error.call(this);if(Error.captureStackTrace)
Error.captureStackTrace(this,this.constructor);this.name=name||"Exception";this.message=message;this.data=data;}
Exception.prototype.__proto__=Error.prototype;Exception.prototype._getStackTrace=function(){var callstack=[];var isCallstackPopulated=false;};Guard={};Guard.requireValue=function(name,value){if(typeof value==='undefined'||value===null){Guard.raise(name+" requires a value other than undefined or null");}};Guard.requireType=function(name,value,typeOrTypes){var types=typeOrTypes instanceof Array?typeOrTypes:[typeOrTypes];return types.some(function(item){switch(typeof item){case"string":return typeof value===item;case"function":return value instanceof item;default:Guard.raise("Unknown type format : "+typeof item+" for: "+name);}});};Guard.raise=function(exception){if(typeof intellisense==='undefined'){if(exception instanceof Exception){console.error(exception.name+':',exception.message+'\n',exception);}else{console.error(exception);}
...
classFunction.resolverThunks=classFunction.resolverThunks||[];classFunction.childResolverThunks=classFunction.childResolverThunks||[];var t=memberDefinition.type;var et=memberDefinition.elementType;function addChildThunk(referencedType){if(referencedType&&referencedType.isAssignableTo&&$data.Entity&&referencedType.isAssignableTo($data.Entity)){classFunction.childResolverThunks.push(function(){if(referencedType.resolveForwardDeclarations){referencedType.resolveForwardDeclarations();}});}}
addChildThunk(t);addChildThunk(et);if("string"===typeof t){if("@"===t[0]){memberDefinition.type=t.substr(1);memberDefinition.dataType=t.substr(1);}else{classFunction.resolverThunks.push(function(){var rt=classFunction.container.resolveType(t);addChildThunk(rt);memberDefinition.type=rt;memberDefinition.dataType=rt;});}}
if(et){if("string"===typeof et){if("@"===et[0]){memberDefinition.elementType=et.substr(1);}else{classFunction.resolverThunks.push(function(){var rt=classFunction.container.resolveType(et);addChildThunk(rt);memberDefinition.elementType=rt;});}}}
classFunction.resolveForwardDeclarations=function(){classFunction.resolveForwardDeclarations=function(){};$data.Trace.log("resolving: "+classFunction.fullName);this.resolverThunks.forEach(function(thunk){thunk();});this.childResolverThunks.forEach(function(thunk){thunk();});}
return memberDefinition;};MemberDefinition.prototype.toJSON=function(){var property={};for(var name in this){if(name!=='defineBy'&&name!=='storageModel'){if((name==='type'||name==='dataType')&&(this[name]&&typeof this[name]==='function')){try{property[name]=Container.resolveName(this[name]);}catch(e){property[name]=this[name];}}else{property[name]=this[name];}}}
return property;}
...
return ret;};this.getTypeName=function(value){switch(typeof value){case'object':if(value==null)return'$data.Object';if(value instanceof Array)return'$data.Array';if(value.getType)return value.getType().fullName;if(value instanceof Date)return'$data.Date';if(value instanceof $data.Guid)return'$data.Guid';if(value instanceof $data.DateTimeOffset)return'$data.DateTimeOffset';if(value instanceof $data.GeographyPoint)return'$data.GeographyPoint';if(value instanceof $data.GeographyLineString)return'$data.GeographyLineString';if(value instanceof $data.GeographyPolygon)return'$data.GeographyPolygon';if(value instanceof $data.GeographyMultiPoint)return'$data.GeographyMultiPoint';if(value instanceof $data.GeographyMultiLineString)return'$data.GeographyMultiLineString';if(value instanceof $data.GeographyMultiPolygon)return'$data.GeographyMultiPolygon';if(value instanceof $data.GeographyCollection)return'$data.GeographyCollection';if(value instanceof $data.GeographyBase)return'$data.GeographyBase';if(value instanceof $data.GeometryPoint)return'$data.GeometryPoint';if(value instanceof $data.GeometryLineString)return'$data.GeometryLineString';if(value instanceof $data.GeometryPolygon)return'$data.GeometryPolygon';if(value instanceof $data.GeometryMultiPoint)return'$data.GeometryMultiPoint';if(value instanceof $data.GeometryMultiLineString)return'$data.GeometryMultiLineString';if(value instanceof $data.GeometryMultiPolygon)return'$data.GeometryMultiPolygon';if(value instanceof $data.GeometryCollection)return'$data.GeometryCollection';if(value instanceof $data.GeometryBase)return'$data.GeometryBase';if(value instanceof $data.Geospatial)return'$data.Geospatial';if(value instanceof $data.SimpleBase)return'$data.SimpleBase';if(typeof value.toHexString==='function')return'$data.ObjectID';default:return typeof value;}};this.isTypeRegistered=function(typeOrName){if(typeof typeOrName==='function'){return classTypes.indexOf(typeOrName)>-1;}else{return typeOrName in classNames;}};this.unregisterType=function(type){Guard.raise("Unimplemented");};this.getDefault=function(typeOrName){var t=this.resolveType(typeOrName);switch(t){case $data.Number:return 0.0;case $data.Float:return 0.0;case $data.Decimal:return'0.0';case $data.Integer:return 0;case $data.Int16:return 0;case $data.Int32:return 0;case $data.Int64:return'0';case $data.Byte:return 0;case $data.SByte:return 0;case $data.String:return null;case $data.Boolean:return false;default:return null;}};this.getIndex=function(typeOrName){var t=this.resolveType(typeOrName);return classTypes.indexOf(t);}
this.resolveByIndex=function(index){return classTypes[index];}
this.registerType=function(nameOrNamesArray,type,factoryFunc){if(!nameOrNamesArray){return;}
...
if(!type.name){type.name=namesArray[0].shortName;}};var _converters={from:{},to:{}};this.converters=_converters;this.convertTo=function(value,tType,eType,options){Guard.requireValue("typeOrName",tType);if(Object.isNullOrUndefined(value))
return value;var sourceTypeName=Container.getTypeName(value);var sourceType=Container.resolveType(sourceTypeName);var sourceTypeName=Container.resolveName(sourceType);var targetType=Container.resolveType(tType);var targetTypeName=Container.resolveName(targetType);var result;try{if(typeof targetType['from'+sourceTypeName]==='function'){result=targetType['from'+sourceTypeName].apply(targetType,arguments);}else if(typeof sourceType['to'+targetTypeName]==='function'){result=sourceType['to'+targetTypeName].apply(sourceType,arguments);}else if(_converters.to[targetTypeName]&&_converters.to[targetTypeName][sourceTypeName]){result=_converters.to[targetTypeName][sourceTypeName].apply(_converters,arguments);}else if(_converters.from[sourceTypeName]&&_converters.from[sourceTypeName][targetTypeName]){result=_converters.from[sourceTypeName][targetTypeName].apply(_converters,arguments);}else if(targetTypeName===sourceTypeName||value instanceof targetType){result=value;}else if(_converters.to[targetTypeName]&&_converters.to[targetTypeName]['default']){result=_converters.to[targetTypeName]['default'].apply(_converters,arguments);}else{throw"converter not found";}}catch(e){Guard.raise(new Exception("Value '"+sourceTypeName+"' not convertable to '"+targetTypeName+"'",'TypeError',value));}
if(targetType===$data.Array&&eType&&Array.isArray(result)){for(var i=0;i<result.length;i++){result[i]=this.convertTo.call(this,result[i],eType,undefined,options);}}
return result;};this.registerConverter=function(target,sourceOrToConverters,toConverterOrFromConverters,fromConverter){var targetName=Container.resolveName(target);if(Container.isTypeRegistered(sourceOrToConverters)){_converters.to[targetName]=_converters.to[targetName]||{};_converters.from[targetName]=_converters.from[targetName]||{};var sourceName=Container.resolveName(sourceOrToConverters);if(toConverterOrFromConverters)
_converters.to[targetName][sourceName]=toConverterOrFromConverters;if(fromConverter)
_converters.from[targetName][sourceName]=fromConverter;}else{if(_converters.to[targetName]){_converters.to[targetName]=$data.typeSystem.extend(_converters.to[targetName],sourceOrToConverters);}else{_converters.to[targetName]=sourceOrToConverters;}
if(_converters.from[targetName]){_converters.from[targetName]=$data.typeSystem.extend(_converters.from[targetName],toConverterOrFromConverters);}else{_converters.from[targetName]=toConverterOrFromConverters;}}};}
$data.ContainerClass=ContainerCtor;var c;global["Container"]=$data.Container=c=global["C$"]=new ContainerCtor();$data.createContainer=function(){return new ContainerCtor($data.Container);}
global["$C"]=function(){Class.define.apply(Class,arguments);};var storeProperty=function(memberDefinition,value){var backingFieldName="_"+memberDefinition.name;if(!this[backingFieldName]){Object.defineProperty(this,backingFieldName,memberDefinition.createStorePropertyDescriptor(value));}
else{this[backingFieldName]=value;}};var retrieveProperty=function(memberDefinition){var backingFieldName="_"+memberDefinition.name;return this[backingFieldName];};$data.Class.define('$data.Base',function Base(){},null,{storeProperty:storeProperty,retrieveProperty:retrieveProperty,setProperty:function(memberDefinition,value,callback){this[memberDefinition.name]=value;callback();},getProperty:function(memberDefinition,callback){callback.apply(this,[this[memberDefinition.name]]);}},{create:function(){return Container.createInstance(this,arguments);},extend:function(name,container,instanceDefinition,classDefinition){if(container&&!(container instanceof ContainerCtor)){classDefinition=instanceDefinition;instanceDefinition=container;container=undefined;}
...
var result;if(callBack==null||callBack==undefined){result=setting;}else if(typeof callBack=='function'){result=this.extend(setting,{success:callBack});}else{result=this.extend(setting,callBack);}
function wrapCode(fn){var t=this;function r(){fn.apply(t,arguments);fn=function(){}}return r;}
if(typeof result.error==='function')
result.error=wrapCode(result.error);return result;},createCtorParams:function(source,indexes,thisObj){if(indexes){var paramArray=[];for(var i=0,l=indexes.length;i<l;i++){var item=i;if(indexes[item]instanceof ConstructorParameter)
...
var result={success:true,errorMessage:'',errorDetails:''};if($data.Acorn){return{success:true,expression:this.ParserBuild($data.Acorn.parse('('+code.toString()+')').body[0]),errors:[]};}else if($data.Esprima){return{success:true,expression:this.ParserBuild($data.Esprima.parse('('+code.toString()+')').body[0]),errors:[]};}else{var AST=$data.ASTParser.parseCode(code);this.log({event:"AST",data:AST});if(!AST.success){return{success:false,error:"ASTParser error",errorMessage:(AST.errors)?JSON.stringify(AST.errors):"could not get code"};}
...
{var result=eNode.implementation(left.value,right.value);return Container.createConstantExpression(result,typeof result);}
...
{var result=eNode.operator.implementation(operand.value);return Container.createConstantExpression(result,typeof result);}
...
var value=eNode.implementation(obj.value,fn,args.map(convertToValue));return new $data.Expressions.ConstantExpression(value,typeof value);}
...
member=Container.createMemberInfoExpression(operation);result=Container.createContextFunctionOperationExpression(exp,member,this._resolveFunctionArguments(args,operation.method.params));return result;break;default:Guard.raise("VisitCall: Only fields can have operations: "+expression.getType().name);}},_resolveFunctionArguments:function(args,params){if(params)
params=params.filter(function(p,i){return p.name!=='@expression';});if(args.length===1&&args[0]instanceof $data.Expressions.ConstantExpression&&typeof args[0].value==='object'&&args[0].value&&params&&params[0]&&args[0].value.constructor===$data.Object&&params.some(function(param){return param.name in args[0].value})){return params.map(function(p){var type=p.type||p.dataType||args[0].type;return new $data.Expressions.ConstantExpression(args[0].value[p.name],Container.resolveType(type),p.name);});}else{return args.map(function(expr,i){if(expr instanceof $data.Expressions.ConstantExpression&&params&&params[i]){var type=params[i].type||params[i].dataType||expr.type;return new $data.Expressions.ConstantExpression(expr.value,Container.resolveType(type),params[i].name);}else{return expr;}});}},VisitProperty:function(expression,context){var exp=this.Visit(expression.expression,context);var member=this.Visit(expression.member,context);Guard.requireType("member",member,$data.Expressions.ConstantExpression);function isPrimitiveType(memberDefinitionArg){var t=memberDefinitionArg.dataType;if(typeof t==='function'){return false;}}
...
return Container.createSomeExpression(source);return expression;},VisitFindExpression:function(expression,context){var source=this.Visit(expression.source,context);if(source!==expression.source)
return Container.createFindExpression(source);return expression;},VisitEveryExpression:function(expression,context){var source=this.Visit(expression.source,context);if(source!==expression.source)
...
return expression;}});$C('$data.Expressions.QueryParameterExpression',$data.Expressions.ExpressionNode,null,{constructor:function(name,index,value,type){this.name=name;this.index=index;this.value=value;this.type=Container.getTypeName(value);},nodeType:{value:$data.Expressions.ExpressionType.QueryParameter,writable:false}});$C('$data.Expressions.RepresentationExpression',$data.Expressions.ExpressionNode,null,{constructor:function(kind){},getMemberDefinition:function(name){return this.entityType.getMemberDefinition(name);},nodeType:{value:$data.Expressions.ExpressionType.Entity}});$C('$data.Expressions.ServiceOperationExpression',$data.Expressions.ExpressionNode,null,{constructor:function(source,selector,params,cfg,boundItem){Guard.requireType("source",source,[$data.Expressions.EntityContextExpression]);Guard.requireType("selector",source,[$data.Expressions.MemberInfoExpression]);this.source=source;this.selector=selector
this.params=params
this.cfg=cfg;this.boundItem=boundItem;function findContext(){var r=source;while(r){if(r instanceof $data.Expressions.EntityContextExpression){return r;}
r=r.source;}}
...
this.changedProperties=undefined;this.entityState=undefined;},toString:function(){return this.getType().fullName+"("+(this.Id||this.Name||'')+")"},toJSON:function(){var result={};var self=this;this.getType().memberDefinitions.getPublicMappedProperties().forEach(function(memDef){if(self[memDef.name]instanceof Date&&memDef.type&&Container.resolveType(memDef.type)===$data.DateTimeOffset){result[memDef.name]=new $data.DateTimeOffset(self[memDef.name]);}else{result[memDef.name]=self[memDef.name];}});return result;},equals:function(entity){if(entity.getType()!==this.getType()){return false;}
var entityPk=this.getType().memberDefinitions.getKeyProperties();for(var i=0;i<entityPk.length;i++){if(this[entityPk[i].name]!=entity[entityPk[i].name]){return false;}}
return true;},propertyChanging:{dataType:$data.Event,storeOnObject:true,monitorChanges:false,notMapped:true,enumerable:false,prototypeProperty:true,get:function(){if(!this._propertyChanging)
...
this._propertyValidationError=new Event('propertyValidationError',this);return this._propertyValidationError;},set:function(value){this._propertyValidationError=value;}},storeProperty:function(memberDefinition,value){if(memberDefinition.concurrencyMode!==$data.ConcurrencyMode.Fixed){value=Container.convertTo(value,memberDefinition.type,memberDefinition.elementType);}
var eventData=null;if(memberDefinition.monitorChanges!=false&&(this._propertyChanging||this._propertyChanged||"instancePropertyChanged"in this.constructor)){var origValue=this[memberDefinition.name];eventData=new PropertyChangeEventData(memberDefinition.name,origValue,value);if(this._propertyChanging)
...
throw'not implemented';},storeToken:{type:Object,monitorChanges:false,notMapped:true,storeOnObject:true},getFieldUrl:function(field){if(this.context){return this.context.getFieldUrl(this,field);}else if(this.getType().storeToken&&typeof this.getType().storeToken.factory==='function'){var context=this.getType().storeToken.factory();return context.getFieldUrl(this,field);}else if(this.getType().storeToken){try{var ctx=$data.ItemStore._getContextPromise('default',this.getType());if(ctx instanceof $data.EntityContext){return ctx.getFieldUrl(this,field);}}catch(e){}}
return'#';}},{__setPropertyfunctions:{value:true,notMapped:true,enumerable:false,storeOnObject:true},__copyPropertiesToInstance:{value:false,notMapped:true,enumerable:false,storeOnObject:true},inheritedTypeProcessor:function(type){if($data.ItemStore&&'EntityInheritedTypeProcessor'in $data.ItemStore)
...
return initData;}}},addEventListener:function(eventName,fn){var delegateName="on"+eventName;if(!(delegateName in this)){this[delegateName]=new $data.Event(eventName,this);}
...
this[delegateName].fire(data);},getFieldNames:function(){return this.memberDefinitions.getPublicMappedPropertyNames();},'from$data.Object':function(value,type,t,options){if(!Object.isNullOrUndefined(value)){var newInstanceOptions;if(options&&options.converters){newInstanceOptions={converters:options.converters}}
return new this(value,newInstanceOptions);}else{return value;}}});$data.define=function(name,container,definition){if(container&&!(container instanceof $data.ContainerClass)){definition=container;container=undefined;}
if(!definition){throw new Error("json object type is not supported yet");}
...
this._storageModel.getStorageModel=function(typeName){var name=Container.resolveName(typeName);return ctx._storageModel[name];};if(typeof storageProviderCfg.name==='string'){var tmp=storageProviderCfg.name;storageProviderCfg.name=[tmp];}
...
if(storageProviderCfg&&storageProviderCfg.user)Object.defineProperty(ctx,'user',{value:storageProviderCfg.user,enumerable:true});if(storageProviderCfg&&storageProviderCfg.checkPermission)Object.defineProperty(ctx,'checkPermission',{value:storageProviderCfg.checkPermission,enumerable:true});ctx._initializeStore(callBack);},error:function(){callBack.error('Provider fallback failed!');}});this.addEventListener=function(eventName,fn){var delegateName="on"+eventName;if(!(delegateName in this)){this[delegateName]=new $data.Event(eventName,this);}
...
this[delegateName].fire(data);};this.ready=this.onReady({success:$data.defaultSuccessCallback,error:function(){if($data.PromiseHandler!==$data.PromiseHandlerBase){$data.defaultErrorCallback.apply(this,arguments);}else{$data.Trace.error(arguments);}}});},beginTransaction:function(){var tables=null;var callBack=null;var isWrite=false;function readParam(value){if(Object.isNullOrUndefined(value))return;if(typeof value==='boolean'){isWrite=value;}else if(Array.isArray(value)){tables=value;}else{callBack=value;}}
readParam(arguments[0]);readParam(arguments[1]);readParam(arguments[2]);var pHandler=new $data.PromiseHandler();callBack=pHandler.createCallback(callBack);this.storageProvider._beginTran(tables,isWrite,callBack);return pHandler.getPromise();},_isReturnTransaction:function(transaction){return transaction instanceof $data.Base||transaction==='returnTransaction';},_applyTransaction:function(scope,cb,args,transaction,isReturnTransaction){if(isReturnTransaction===true){if(transaction instanceof $data.Transaction){Array.prototype.push.call(args,transaction);cb.apply(scope,args);}else{this.beginTransaction(function(tran){Array.prototype.push.call(args,tran);cb.apply(scope,args);});}}
...
this._storageModel.push(storageModel);var name=Container.resolveName(elementType);this._storageModel[name]=storageModel;}}}},_initializeStorageModel:function(){var _memDefArray=this.getType().memberDefinitions.asArray();if(typeof intellisense!=='undefined')
...
var refereedStorageModel=this._storageModel.getStorageModel(refereedType);if(!refereedStorageModel){if(typeof intellisense==='undefined'){Guard.raise(new Exception("No EntitySet definition for the following element type","Field definition",memDef));}}
...
var refereedStorageModel=this._storageModel.getStorageModel(refereedType);;if(!refereedStorageModel){if(typeof intellisense==='undefined'){Guard.raise(new Exception("No EntitySet definition following element type","Field definition",memDef));}}
...
return pHandler.getPromise();},ready:{type:$data.Promise},getEntitySetFromElementType:function(elementType){var result=this._entitySetReferences[elementType];if(!result){try{result=this._entitySetReferences[eval(elementType).name];}catch(ex){}}
return result;},executeQuery:function(queryable,callBack,transaction){var query=new $data.Query(queryable.expression,queryable.defaultType,this);query.transaction=transaction instanceof $data.Transaction?transaction:undefined;var returnTransaction=this._isReturnTransaction(transaction);callBack=$data.typeSystem.createCallbackSetting(callBack);var that=this;var clbWrapper={};clbWrapper.success=function(query){if($data.QueryCache&&$data.QueryCache.isCacheable(that,query)){$data.QueryCache.addToCache(that,query);}
query.buildResultSet(that);if($data.ItemStore&&'QueryResultModifier'in $data.ItemStore)
...
for(var j=0;j<navigationProperties.length;j++){var navProp=navigationProperties[j];var association=sModel.Associations[navProp.name];var name=navProp.name;var navPropertyName=association.ToPropertyName;var connectedDataList=[].concat(entityCachedItem.data[name]);for(var k=0;k<connectedDataList.length;k++){var data=connectedDataList[k];if(data){var value=data[navPropertyName];var associationType=association.FromMultiplicity+association.ToMultiplicity;if(association.FromMultiplicity==='$$unbound'){if(data instanceof $data.Array){entityCachedItem.dependentOn=entityCachedItem.dependentOn||[];for(var l=0;l<data.length;l++){var dataItem=data[l];if((entityCachedItem.dependentOn.indexOf(data)<0)&&(data.skipSave!==true)){entityCachedItem.dependentOn.push(data);}}}else{entityCachedItem.dependentOn=entityCachedItem.dependentOn||[];if((entityCachedItem.dependentOn.indexOf(data)<0)&&(data.skipSave!==true)){entityCachedItem.dependentOn.push(data);}}}else{switch(associationType){case"*0..1":if(value){if(value instanceof Array){if(value.indexOf(entityCachedItem.data)==-1){value.push(entityCachedItem.data);data.initData[navPropertyName]=value;data._setPropertyChanged(association.ToType.getMemberDefinition(navPropertyName));}}else{if(typeof intellisense==='undefined'){Guard.raise("Item must be array or subtype of array");}}}else{data.initData[navPropertyName]=[entityCachedItem.data];data._setPropertyChanged(association.ToType.getMemberDefinition(navPropertyName));}
break;default:if(value){if(value!==entityCachedItem.data){if(typeof intellisense==='undefined'){Guard.raise("Integrity check error! Item assigned to another entity!");}}}else{data.initData[navPropertyName]=entityCachedItem.data;data._setPropertyChanged(association.ToType.getMemberDefinition(navPropertyName));}
break;}
...
break;}}
if(!data.entityState){if(data.storeToken===this.storeToken){data.entityState=$data.EntityState.Modified;}else{data.entityState=$data.EntityState.Added;}}
...
var errors=[];for(var i=0;i<changedEntities.length;i++){var entity=changedEntities[i];if(entity.data.entityState===$data.EntityState.Added){for(var j=0;j<entity.data.getType().memberDefinitions.getPublicMappedProperties().length;j++){var memDef=entity.data.getType().memberDefinitions.getPublicMappedProperties()[j];var memDefType=Container.resolveType(memDef.type);if(memDef.required&&!memDef.computed&&!entity.data[memDef.name]&&!memDef.isDependentProperty){switch(memDefType){case $data.String:case $data.Number:case $data.Float:case $data.Decimal:case $data.Integer:case $data.Int16:case $data.Int32:case $data.Int64:case $data.Byte:case $data.SByte:case $data.Date:case $data.Boolean:entity.data[memDef.name]=Container.getDefault(memDef.dataType);break;default:break;}}}}
...
break;default:break;}}},processEntityTypeAfterEventHandler:function(entityCachedItem){var entity=entityCachedItem.data;var entityType=entity.getType();var state=entity.entityState;switch(true){case state===$data.EntityState.Added&&entityType.onafterCreate instanceof $data.Event:entityType.onafterCreate.fire(entity);break;case state===$data.EntityState.Modified&&entityType.onafterUpdate instanceof $data.Event:entityType.onafterUpdate.fire(entity);break;case state===$data.EntityState.Deleted&&entityType.onafterDelete instanceof $data.Event:entityType.onafterDelete.fire(entity);break;default:break;}},bulkInsert:function(entitySet,fields,datas,callback){var pHandler=new $data.PromiseHandler();callback=pHandler.createCallback(callback);if(typeof entitySet==='string'){var currentEntitySet;for(var entitySetName in this._entitySetReferences){var actualEntitySet=this._entitySetReferences[entitySetName];if(actualEntitySet.tableName===entitySet){currentEntitySet=actualEntitySet;break;}}
if(!currentEntitySet)
currentEntitySet=this[entitySet];entitySet=currentEntitySet;}
if(entitySet){this.storageProvider.bulkInsert(entitySet,fields,datas,callback);}else{callback.error(new Exception('EntitySet not found'));}
return pHandler.getPromise();},prepareRequest:function(){},_postProcessSavedItems:function(callBack,changedEntities,transaction,returnTransaction){if(this.ChangeCollector&&this.ChangeCollector instanceof $data.Notifications.ChangeCollectorBase)
this.ChangeCollector.processChangedData(changedEntities);var eventData={};var ctx=this;for(var i=0;i<changedEntities.length;i++){var entity=changedEntities[i];if(!entity.data.storeToken)
entity.data.storeToken=ctx.storeToken;this.processEntityTypeAfterEventHandler(entity);var oes=entity.data.entityState;entity.data.entityState=$data.EntityState.Unchanged;entity.data.changedProperties=[];entity.physicalData=undefined;var n=entity.entitySet.elementType.name;var es=ctx._entitySetReferences[n];var eventName=undefined;switch(oes){case $data.EntityState.Added:eventName='added';break;case $data.EntityState.Deleted:eventName='deleted';break;case $data.EntityState.Modified:eventName='updated';break;}
...
var tempOperation=$data.EntityContext.generateServiceOperation({serviceName:functionName,returnType:$data.Queryable,elementType:this[returnEntitySet].elementType,params:params});return tempOperation.apply(this,arg);},attach:function(entity,mode){if(entity instanceof $data.EntityWrapper){entity=entity.getEntity();}
var entitySet=this.getEntitySetFromElementType(entity.getType());return entitySet.attach(entity,mode);},attachOrGet:function(entity,mode){if(entity instanceof $data.EntityWrapper){entity=entity.getEntity();}
var entitySet=this.getEntitySetFromElementType(entity.getType());return entitySet.attachOrGet(entity,mode);},addMany:function(entities){var self=this;entities.forEach(function(entity){self.add(entity);});return entities;},add:function(entity){if(entity instanceof $data.EntityWrapper){entity=entity.getEntity();}
...
var entitySet=this.getEntitySetFromElementType(entity.getType());return entitySet.remove(entity);},storeToken:{type:Object},getFieldUrl:function(entity,member,collection){try{var entitySet=typeof collection==='string'?this[collection]:collection;var fieldName=typeof member==='string'?member:member.name;if(entity instanceof $data.Entity){entitySet=this.getEntitySetFromElementType(entity.getType());}else if(!Object.isNullOrUndefined(entity)&&entity.constructor!==$data.Object){var keyDef=entitySet.elementType.memberDefinitions.getKeyProperties()[0];var key={};key[keyDef.name]=entity;entity=key;}
if(!(entity instanceof $data.Entity)){entity=new entitySet.elementType(entity);}
return this.storageProvider.getFieldUrl(entity,fieldName,entitySet);}catch(e){}
return'#';}},{inheritedTypeProcessor:function(type){if(type.resolveForwardDeclarations){type.resolveForwardDeclarations();}},generateServiceOperation:function(cfg){var fn;if(cfg.serviceMethod){var returnType=cfg.returnType?Container.resolveType(cfg.returnType):{};if(returnType.isAssignableTo&&returnType.isAssignableTo($data.Queryable)){fn=cfg.serviceMethod;}else{fn=function(){var lastParam=arguments[arguments.length-1];var pHandler=new $data.PromiseHandler();var cbWrapper;var args=arguments;if(typeof lastParam==='function'){cbWrapper=pHandler.createCallback(lastParam);arguments[arguments.length-1]=cbWrapper;}else{cbWrapper=pHandler.createCallback();arguments.push(cbWrapper);}
...
return pHandler.getPromise();}}}else{fn=function(){var context=this;var boundItem;if(this instanceof $data.Entity){if(!cfg.method){cfg.method='POST';}
if(this.context){context=this.context;}else{Guard.raise('entity not attached into context');return;}
boundItem={data:this,entitySet:context.getEntitySetFromElementType(this.getType())};}
...
return this._finalize(o);},_finalize:function(o){if(o instanceof $data.Entity){o.changedProperties=undefined;o.storeToken=this.context.storeToken;}
return o;},_buildSelector:function(meta,context){if(meta.$selector){if(!(Array.isArray(meta.$selector))){meta.$selector=[meta.$selector];}
...
var resolvedType=Container.resolveType(meta.$type);var type=Container.resolveName(resolvedType);var typeIndex=Container.getIndex(resolvedType);var isEntityType=resolvedType.isAssignableTo&&resolvedType.isAssignableTo($data.Entity);var item='_'+type.replace(/\./gi,'_')+'_';if(context.item==item)item+='new_';context.item=item;var isPrimitive=false;if(!meta.$source&&!meta.$value&&resolvedType!==$data.Array&&resolvedType!==$data.Object&&!resolvedType.isAssignableTo)
...
if(expression.source)fn(expression.source);};fn(this.expression);return ret;}},null);$data.Class.define('$data.Queryable',null,null,{constructor:function(source,rootExpression){var context=source instanceof $data.EntityContext?source:source.entityContext;this.defaultType=source instanceof $data.EntityContext?null:source.defaultType;this.entityContext=context;this.expression=rootExpression;},filter:function(predicate,thisArg){if(arguments.length===3){predicate="it."+arguments[0]+
(arguments[1][0]==="."?(arguments[1]+"(param)"):(" "+arguments[1]+" param"));thisArg={param:arguments[2]}}
this._checkOperation('filter');var expression=Container.createCodeExpression(predicate,thisArg);var expressionSource=this.expression;if(this.expression instanceof $data.Expressions.FilterExpression){expressionSource=this.expression.source;var operatorResolution=this.entityContext.storageProvider.resolveBinaryOperator("and");expression=Container.createSimpleBinaryExpression(this.expression.selector,expression,"and","filter","boolean",operatorResolution);}
...
return pHandler.getPromise();},toLiveArray:function(onResult,transaction){var self=this;var result=[];var doAction=function(action){return function(onResult){var pHandler=new $data.PromiseHandler();var callback=pHandler.createCallback(onResult);var successFunc=function(res){result.length=0;var data=res;$data.typeSystem.extend(result,data);result.prev=doAction(function(cb){data.prev(cb);});result.next=doAction(function(cb){data.next(cb);});callback.success.apply(this,[result].concat(Array.prototype.slice.call(arguments,1)));}
action({success:successFunc,error:callback.error},transaction);var promise=pHandler.getPromise();$data.typeSystem.extend(result,promise);return result;}}
result.refresh=doAction(function(cb){self.toArray(cb);});return result.refresh.apply(result,arguments);},single:function(filterPredicate,thisArg,onResult,transaction){this._checkOperation('single');var q=this;if(filterPredicate){q=this.filter(filterPredicate,thisArg);}
...
return pHandler.getPromise();},find:function(keyValue,onResult,transaction){var pHandler=new $data.PromiseHandler();var cbWrapper=pHandler.createCallback(onResult);var keys=this.defaultType.memberDefinitions.getKeyProperties();try{if(keys.length===1&&typeof keyValue!=='object'){var keyV={};keyV[keys[0].name]=keyValue;keyValue=keyV;}
if(typeof keyValue!=='object'){throw new Exception('Key parameter is invalid');}else{var parameters=[];for(var i=0;i<keys.length;i++){var keyProp=keys[i];if(!(keyProp.name in keyValue)){throw new Exception('Key value missing');}
parameters.push(Container.createConstantExpression(keyValue[keyProp.name],keyProp.type,keyProp.name));}
var operation=this.entityContext.storageProvider.supportedSetOperations['find'];if(operation){var findExpression=Container.createFindExpression(this.expression,parameters);var preparator=Container.createQueryExpressionCreator(this.entityContext);try{var expression=preparator.Visit(findExpression);this.entityContext.log({event:"EntityExpression",data:expression});this.entityContext.executeQuery(Container.createQueryable(this,expression),cbWrapper,transaction);}catch(e){cbWrapper.error(e);}}else{var predicate='';var params={}
for(var i=0;i<parameters.length;i++){var param=parameters[i];params[param.name]=param.value;if(i>0)predicate+=' && ';predicate+="it."+param.name+" == this."+param.name;}
this.single(predicate,params,cbWrapper,transaction);}}}catch(e){cbWrapper.error(e);}
...
data.entityState=$data.EntityState.Deleted;data.changedProperties=undefined;this._trackEntity(data);},attach:function(entity,mode){var data=entity;if(entity instanceof $data.EntityWrapper){data=entity.getEntity();}else if(!(entity instanceof this.createNew)){data=new this.createNew(entity);}
...
if(mode===true){if(data.changedProperties&&data.changedProperties.length>0){data.entityState=$data.EntityState.Modified;}else{data.entityState=$data.EntityState.Unchanged;}}else{if(typeof mode==="string")mode=$data.EntityAttachMode[mode];var attachMode=mode||$data.EntityAttachMode[$data.EntityAttachMode.defaultMode];if(typeof attachMode==="function"){attachMode.call($data.EntityAttachMode,data);}else{data.entityState=$data.EntityState.Unchanged;data.changedProperties=undefined;}}
...
contextDef[storeConfig.collectionName]['tableName']=storeConfig.tableName;var inMemoryType=$data.EntityContext.extend(typeName,contextDef);var ctx=new inMemoryType(storeConfig.initParam);if(initStoreConfig&&typeof initStoreConfig==='object')
initStoreConfig.factory=ctx._storeToken.factory;return ctx;}
...
$data.Trace.log(providerName+' provider is '+(supported?'':'not')+' supported');return supported;},scriptLoadTimeout:{type:'int',value:1000},scriptLoadInterval:{type:'int',value:50},npmModules:{value:{'indexedDb':'jaydata-indexeddb','InMemory':'jaydata-inmemory','LocalStore':'jaydata-inmemory','mongoDB':'jaydata-mongodb','oData':'jaydata-odata','webApi':'jaydata-webapi','sqLite':'jaydata-sqlite','webSql':'jaydata-sqlite','storm':'jaydata-storm'}},ProviderNames:{value:{'indexedDb':'IndexedDb','InMemory':'InMemory','LocalStore':'InMemory','oData':'oData','webApi':'WebApi','sqLite':'SqLite','webSql':'SqLite','storm':'Storm'}},load:function(providerList,callback){$data.RegisteredStorageProviders=$data.RegisteredStorageProviders||{};$data.Trace.log('Loading provider(s): '+providerList);callback=$data.typeSystem.createCallbackSetting(callback);var self=this;var cacheKey=providerList.join(',');self._fallbackCache=self._fallbackCache||{};if(self._fallbackCache[cacheKey]){callback.success(self._fallbackCache[cacheKey]);}else{this.find(providerList,{success:function(provider,selectedProvider){self._fallbackCache[cacheKey]=provider;callback.success.call(this,provider);},error:callback.error});}},find:function(providerList,callback){var currentProvider=providerList.shift();var currentProvider=this.getVirtual(currentProvider);if(Array.isArray(currentProvider)){providerList=currentProvider;currentProvider=providerList.shift();}
while(currentProvider&&!this.isSupported(currentProvider)){currentProvider=providerList.shift();}
...
return new XMLHttpRequest();else if(window.ActiveXObject!==undefined)
...
foreignType.memberDefinitions.getPublicMappedProperties().filter(function(d){return d.key}).forEach(function(d,i){var constraint=buildDbType_createConstrain(foreignType,dataType,d.name,foreignPropName,keyProperties[i]);if(addToEntityDef){instanceDefinition[constraint[dataType.name]]=buildDbType_copyPropertyDefinition(d,foreignPropName);var dependentMemDef=dataType.getMemberDefinition(keyProperties[i]);if(dependentMemDef){dependentMemDef.isDependentProperty=true;dependentMemDef.navigationPropertyName=association.FromPropertyName;}}
association.ReferentialConstraint.push(constraint);},this);},this);}
...
var ec=Container.createEntityContextExpression(context);if(!memberdef){if(boundItem&&boundItem.data){memberdef=boundItem.data.getType().getMemberDefinition(cfg.serviceName);}else{memberdef=context.getType().getMemberDefinition(cfg.serviceName);}}
var es=Container.createServiceOperationExpression(ec,Container.createMemberInfoExpression(memberdef),paramConstExpression,cfg,boundItem);var clb=arguments[arguments.length-1];if(!(typeof clb==='function'||(typeof clb==='object'&&(typeof clb.success==='function'||typeof clb.error==='function')))){clb=undefined;}
if(virtualEntitySet){var q=Container.createQueryable(virtualEntitySet,es);if(clb){es.isTerminated=true;return q._runQuery(clb);}
return q;}
else{var returnType=cfg.returnType?Container.resolveType(cfg.returnType):null;var q=Container.createQueryable(context,es);q.defaultType=returnType||$data.Object;if(returnType===$data.Queryable){q.defaultType=Container.resolveType(cfg.elementType);if(clb){es.isTerminated=true;return q._runQuery(clb);}
return q;}
es.isTerminated=true;return q._runQuery(clb);}};};var params=cfg.params||[];$data.typeSystem.extend(fn,cfg,{params:params});return fn;}});$data.Class.define('$data.ServiceAction',$data.ServiceOperation,null,{},{generateServiceOperation:function(cfg){if(!cfg.method){cfg.method='POST';}
return $data.ServiceOperation.generateServiceOperation.apply(this,arguments);}});$data.Base.extend('$data.EntityWrapper',{getEntity:function(){Guard.raise("pure object");}});if(typeof jQuery!=='undefined'&&jQuery.ajax){$data.ajax=$data.ajax||jQuery.ajax;}
...
builder.modelBinderConfig['$item']={};builder.selectModelBinderProperty('$item');this.VisitExpression(expression,builder);builder.resetModelBinderProperty();this._query.modelBinderConfig=builder.modelBinderConfig;},_addPropertyToModelBinderConfig:function(elementType,builder){var storageModel=this._query.context._storageModel.getStorageModel(elementType);if(elementType.memberDefinitions){elementType.memberDefinitions.getPublicMappedProperties().forEach(function(prop){if((!storageModel)||(storageModel&&!storageModel.Associations[prop.name]&&!storageModel.ComplexTypes[prop.name])){var type=Container.resolveType(prop.dataType);if(!storageModel&&this._query.context.storageProvider.supportedDataTypes.indexOf(type)<0){builder.selectModelBinderProperty(prop.name);builder.modelBinderConfig['$type']=type;if(this._isoDataProvider){builder.modelBinderConfig['$selector']=['json:'+prop.name+'.results','json:'+prop.name];}else{builder.modelBinderConfig['$selector']='json:'+prop.name;}
this._addPropertyToModelBinderConfig(type,builder);builder.popModelBinderProperty();}else{if(prop.key){builder.addKeyField(prop.name);}
if(prop.concurrencyMode===$data.ConcurrencyMode.Fixed){builder.modelBinderConfig[prop.name]={$selector:'json:__metadata',$source:'etag'}}else if(type===$data.Array&&prop.elementType){builder.selectModelBinderProperty(prop.name);builder.modelBinderConfig['$type']=type;if(this._isoDataProvider){builder.modelBinderConfig['$selector']=['json:'+prop.name+'.results','json:'+prop.name];}else{builder.modelBinderConfig['$selector']='json:'+prop.name;}
builder.selectModelBinderProperty('$item');var arrayElementType=Container.resolveType(prop.elementType);builder.modelBinderConfig['$type']=arrayElementType;this._addPropertyToModelBinderConfig(arrayElementType,builder);builder.popModelBinderProperty();builder.popModelBinderProperty();}else{builder.modelBinderConfig[prop.name]=prop.name;}}}},this);}else{builder._binderConfig.$item=builder._binderConfig.$item||{};builder.modelBinderConfig=builder._binderConfig.$item;}
...
var factoryFn=self.createFactoryFunc(ctxType,cnf,versionInfo);this.factoryCache[cnf.url]=[factoryFn,ctxType];factoryFn.type=ctxType;if(self.debugMode){factoryFn.codeText=codeText;callBack.success(factoryFn,ctxType,codeText);}
else{callBack.success(factoryFn,ctxType);}},_loadXMLDoc:function(cnf,callback){var that=this;if($data.postMessageODataHandler){if(cnf.user&&cnf.password&&(!cnf.httpHeaders||(cnf.httpHeaders&&!cnf.httpHeaders['Authorization']))){httpHeader=httpHeader||{};httpHeader["Authorization"]="Basic "+this.__encodeBase64(cnf.user+":"+cnf.password);}
$data.postMessageODataHandler.requestProxy({url:cnf.metadataUri,httpHeaders:cnf.httpHeaders,success:function(response){var doc;if(typeof module!=='undefined'&&typeof require!=='undefined'){doc=response.responseText;}else if(window.ActiveXObject){doc=new ActiveXObject('Microsoft.XMLDOM');doc.async='false';doc.loadXML(response.responseText);}else{var parser=new DOMParser();doc=parser.parseFromString(response.responseText,'text/xml');}
callback(doc,response);},error:function(e){that._loadXHTTP_XMLDoc(cnf,callback);}});}else{this._loadXHTTP_XMLDoc(cnf,callback);}},_loadXHTTP_XMLDoc:function(cnf,callback){var xhttp=new XMLHttpRequest();xhttp.open("GET",cnf.metadataUri,true);if(cnf.httpHeaders){Object.keys(cnf.httpHeaders).forEach(function(header){xhttp.setRequestHeader(header,cnf.httpHeaders[header]);});}
...
return allowedTypes;},_discoverType:function(typeData,doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys,collectTypes,collectedTypes){var typeName=typeData.Name;if(allowedTypeNames.indexOf(typeName)>=0)
{return;}
console.log("Discover: "+typeName);var typeShortName=typeName;var typeNamespace='';if(typeName.lastIndexOf('.')>0)
{typeNamespace=typeName.substring(0,typeName.lastIndexOf('.'));typeShortName=typeName.substring(typeName.lastIndexOf('.')+1);}
var schemaNode=doc.querySelector("Schema[Namespace = '"+typeNamespace+"']");if(schemaNode!=null)
{var typeNode=schemaNode.querySelector("EntityType[Name = '"+typeShortName+"'], ComplexType[Name = '"+typeShortName+"']");if(typeNode!=null)
{allowedTypes.push(typeData);allowedTypeNames.push(typeName);if(withKeys&&typeData.Fields.length>0){var keys=typeNode.querySelectorAll("Key PropertyRef");if(keys!=null)
{for(var j=0;j<keys.length;j++)
{var keyField=keys[j].attributes["Name"].value;if(typeData.Fields.indexOf(keyField)<0)
typeData.Fields.splice(j,0,keyField);}}}
if(withNavPropertis)
{var navPropNodes=typeNode.querySelectorAll("NavigationProperty");for(var j=0;j<navPropNodes.length;j++)
{var navProp=navPropNodes[j];if(typeData.Fields.length==0||typeData.Fields.indexOf(navProp.attributes["Name"].value)>=0)
{var FromRole=navProp.attributes["FromRole"].value;var ToRole=navProp.attributes["ToRole"].value;var association=schemaNode.querySelector("Association End[Role = '"+FromRole+"']:not([Type = '"+typeName+"'])");if(association==null)
{association=schemaNode.querySelector("Association End[Role = '"+ToRole+"']:not([Type = '"+typeName+"'])");}
if(association!=null)
{var nav_type=association.attributes["Type"].value;if(collectTypes)
{if(collectedTypes.indexOf(nav_type)<0&&allowedTypeNames.indexOf(nav_type)<0)
collectedTypes.push(nav_type);}
else
{this._discoverType({Name:nav_type,Fields:[]},doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys,false,collectedTypes);}}}}}}}},_discoverProperyDependencies:function(types,doc,withNavPropertis,withKeys){var allowedTypes=[];var allowedTypeNames=types.map(function(t){return t.Name;});for(var i=0;i<types.length;i++)
{this._discoverProperties(types[i],doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys);}
return allowedTypes;},_discoverProperties:function(typeData,doc,allowedTypes,allowedTypeNames,withNavPropertis,withKeys){var typeName=typeData.Name;console.log("Discover: "+typeName);var hasProperty=typeData.Fields.length!=0;var typeShortName=typeName;var typeNamespace='';if(typeName.lastIndexOf('.')>0)
{typeNamespace=typeName.substring(0,typeName.lastIndexOf('.'));typeShortName=typeName.substring(typeName.lastIndexOf('.')+1);}
var schemaNode=doc.querySelector("Schema[Namespace = '"+typeNamespace+"']");if(schemaNode!=null)
{var typeNode=schemaNode.querySelector("EntityType[Name = '"+typeShortName+"'], ComplexType[Name = '"+typeShortName+"']");if(typeNode!=null)
{allowedTypes.push(typeData);if(!hasProperty)
{var properties=typeNode.querySelectorAll("Property");if(properties!=null)
{for(var j=0;j<properties.length;j++)
{var field=properties[j].attributes["Name"].value;typeData.Fields.push(field);}}
if(withNavPropertis)
{var navPropNodes=typeNode.querySelectorAll("NavigationProperty");for(var j=0;j<navPropNodes.length;j++)
{var navProp=navPropNodes[j];var nav_name=navProp.attributes["Name"].value;var types=[navProp.attributes["FromRole"].value,navProp.attributes["ToRole"].value];var nav_type='';for(var t=0;t<types.length;t++)
{var association=schemaNode.querySelector("Association End[Role = '"+types[t]+"']");if(association!=null)
{nav_type=association.attributes["Type"].value;if(nav_type!=typeName||t==1)
break;}}
if(allowedTypeNames.indexOf(nav_type)>=0)
{typeData.Fields.push(nav_name);}}}}
else if(withKeys)
{var keys=typeNode.querySelectorAll("Key PropertyRef");if(keys!=null)
{for(var j=0;j<keys.length;j++)
{var keyField=keys[j].attributes["Name"].value;if(typeData.Fields.indexOf(keyField)<0)
typeData.Fields.splice(j,0,keyField);}}}}}},_findVersion:function(metadata){var maxDSVersion='';if(typeof metadata==='object'&&"getElementsByTagName"in metadata){var version='http://schemas.microsoft.com/ado/2008/09/edm';var item=metadata.getElementsByTagName('Schema');if(item)
item=item[0];if(item)
...
maxDSVersion=maxDSVersion.attributes.getNamedItem('m:MaxDataServiceVersion');if(maxDSVersion&&version)
maxDSVersion=maxDSVersion.value;var versionNum=this._supportedODataVersions[version];return{ns:version,version:versionNum||'unknown',maxVersion:maxDSVersion||this._maxDataServiceVersions[version||'unknown']};}else if(typeof module!=='undefined'&&typeof require!=='undefined'){var schemaXml=metadata;var schemaNamespace='http://schemas.microsoft.com/ado/2008/09/edm';var version='nodejs';for(var i in this._supportedODataVersions){if(schemaXml.search(new RegExp('<Schema.+xmlns=\"'+i+'\"','gi'))>=0){schemaNamespace=i;version=this._supportedODataVersions[i];break;}}
return{ns:schemaNamespace,version:version,maxVersion:this._maxDataServiceVersions[version||'unknown']}}},_supportedODataVersions:{value:{"http://schemas.microsoft.com/ado/2006/04/edm":"V1","http://schemas.microsoft.com/ado/2008/09/edm":"V2","http://schemas.microsoft.com/ado/2009/11/edm":"V3","http://schemas.microsoft.com/ado/2007/05/edm":"V11","http://schemas.microsoft.com/ado/2009/08/edm":"V22"}},_maxDataServiceVersions:{value:{"http://schemas.microsoft.com/ado/2006/04/edm":"2.0","http://schemas.microsoft.com/ado/2008/09/edm":"2.0","http://schemas.microsoft.com/ado/2009/11/edm":"3.0","http://schemas.microsoft.com/ado/2007/05/edm":"2.0","http://schemas.microsoft.com/ado/2009/08/edm":"2.0"}},_supportedODataVersionXSLT:{value:"JayDataContextGenerator.xslt"},getCurrentXSLTVersion:function(versionInfo,metadata){return this._metadataConverterXSLT.replace('@@VERSIONNS@@',versionInfo.ns).replace('@@VERSION@@',versionInfo.version);},__encodeBase64:function(val){var b64array="ABCDEFGHIJKLMNOP"+"QRSTUVWXYZabcdef"+"ghijklmnopqrstuv"+"wxyz0123456789+/"+"=";var input=val;var base64="";var hex="";var chr1,chr2,chr3="";var enc1,enc2,enc3,enc4="";var i=0;do{chr1=input.charCodeAt(i++);chr2=input.charCodeAt(i++);chr3=input.charCodeAt(i++);enc1=chr1>>2;enc2=((chr1&3)<<4)|(chr2>>4);enc3=((chr2&15)<<2)|(chr3>>6);enc4=chr3&63;if(isNaN(chr2)){enc3=enc4=64;}else if(isNaN(chr3)){enc4=64;}
base64=base64+
b64array.charAt(enc1)+
b64array.charAt(enc2)+
b64array.charAt(enc3)+
...
$data.service(apiKey,cfg).then(function(factory){var ctx=factory();return ctx.onReady().then(function(context){context.serviceFactory=factory;d.deferred.resolve(context,factory,factory.type);}).fail(function(){d.deferred.reject.apply(d.deferred,arguments);});}).fail(function(){d.deferred.reject.apply(d.deferred,arguments);});return d.getPromise();};})($data);