...
else{for(var i=0;i<networks.length;i++){item=networks[i];var nw=helpers.networkDefs[item.name];nw.cb=function(newElement){queue.push(newElement)};var reqUrl=nw.url;reqUrl=reqUrl.replace("|id|",item.id);reqUrl=reqUrl.replace("|areaName|",item.areaName);reqUrl=reqUrl.replace("|apiKey|",item.apiKey);reqUrl=reqUrl.replace("|num|",settings.maxResults);processList.push(helpers.doRequest(reqUrl,nw.dataType,nw.cb,nw.parser,settings));}}
...
var helpers={parseResults:function(apiParser,data,settings){var container=$('<div></div>');apiParser.resultsSelector=apiParser.resultsSelector.replace('|num|',settings.maxResults);$.each(eval(apiParser.resultsSelector),function(i,item){var $elem=$(item),heading,txt,linkHref,imgSrc,imgHref,imgAlt,date;try{if(eval(apiParser.preCondition)){var $div=$('<div class="socialist"></div>');$div.addClass('socialist-'+apiParser.name);if(settings.fixed){$div.addClass('socialist-fixed');}
...
txt=eval(apiParser.txtSelector);if(txt!==null){txt=helpers.shorten(txt,settings.textLength);}
else{txt="";}
linkHref="#";if(apiParser.imgSrcSelector===null){imgSrc=apiParser.imgSrc;}
...
catch(e){}});return container;},doRequest:function(url,dataType,cb,parser,settings){return $.ajax({url:url,type:"GET",dataType:dataType,success:function(data){if(parser)
cb($(helpers.parseResults(parser,data,settings)));else
cb(data);},error:function(status){cb($('<div></div>'));}});},buildItem:function(itemObj,container,fields){var $headDiv=$('<div class="head"/>'),$source=$('<div class="source"></div>'),$sourceLnk=$('<a href="'+itemObj.img.href+'" title="'+itemObj.link.title+'"></a>'),$sourceLnkDiv=$('<div/>'),$apiSpan=$('<div class="api"></div>'),$apiSpanLnk=$('<a href="'+itemObj.img.href+'"></a>'),$contentDiv=$('<div class="content"/>'),$contentDivInner=$('<div>'+itemObj.txt+' </div>'),$imgLnk=$('<a href="'+itemObj.img.href+'" title="'+itemObj.link.title+'"></a>'),$img=$('<image src="'+itemObj.img.src+'" alt="'+helpers.stripHtml(itemObj.img.alt)+'">'),$shareDiv=$('<div class="share"><a href="#" title='+itemObj.api+'>fb</a>|<a href="#" class="x">tw</a></div>'),$dateSpan=$('<div class="date"/>'),$footDiv=$('<div class="foot"/>');if(fields.indexOf('image')!=-1&&itemObj.img.src){$img.appendTo($imgLnk);$imgLnk.appendTo($contentDiv);}
...
return time;},fixTwitterDate:function(created_at){var months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];var pattern=/\s/;var day_of_week,day,month_pos,month,year,time;created_at=created_at.split(pattern);for(var i=0;i<created_at.length;i++){day_of_week=created_at[0];day=created_at[2];month_pos=created_at[1];month=0+months.indexOf(month_pos)+1;year=created_at[5];time=created_at[3];}
...
var c=this,d=c.find("img").add(c.filter("img")),e=d.length,f="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==",g=[];return e||h(),d.bind("load.imagesLoaded error.imagesLoaded",i).each(function(){var a=this.src;this.src=f,this.src=a}),c};var w=function(b){a.console&&a.console.error(b)};b.fn.isotope=function(a,c){if(typeof a=="string"){var d=Array.prototype.slice.call(arguments,1);this.each(function(){var c=b.data(this,"isotope");if(!c){w("cannot call methods on isotope prior to initialization; attempted to call method '"+a+"'");return}
