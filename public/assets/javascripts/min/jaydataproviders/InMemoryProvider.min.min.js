...
a.toISOString()+"'))":a},"$data.Time":function(a){return a?"'"+a+"'":a},"$data.String":function(a){return"'"+a.replace(/'/g,"''")+"'"},"$data.Boolean":function(a){return a?"true":"false"},"$data.Blob":function(a){return"'"+$data.Blob.toString(a)+"'"},"$data.Object":function(a){return JSON.stringify(a)},"$data.Array":function(a){return JSON.stringify(a)},"$data.Guid":function(a){return a?"'"+a.toString()+"'":a}}};$C("$data.storageProviders.InMemory.InMemoryProvider",$data.StorageProviderBase,null,{constructor:function(a,b){this.context=b;this.providerConfiguration=$data.typeSystem.extend({source:null,persistentData:!1,localStoreName:"JayData_InMemory_Provider",databaseName:"JayData_InMemory_Provider",__instaceId:$data.createGuid().toString()},a);this.dataSource=this.providerConfiguration.source;delete this.providerConfiguration.source;"JayData_InMemory_Provider"===this.providerConfiguration.databaseName&&(this.providerConfiguration.databaseName=this.providerConfiguration.localStoreName)},initializeStore:function(a){var a=$data.typeSystem.createCallbackSetting(a),b=[],c;for(c in this.context._entitySetReferences)b.push(this.context._entitySetReferences[c].collectionName);var e=null;if(this.providerConfiguration.persistentData&&window.localStorage&&this.providerConfiguration.dbCreation!==$data.storageProviders.DbCreationType.DropAllExistingTables){var d=this,g=window.localStorage.getItem(this.providerConfiguration.databaseName||"JayData_InMemory_Provider");Object.isNullOrUndefined(g)||(e=JSON.parse(g,function(a,c){return-1<b.indexOf(a)&&c.map?c.map(function(b){return new d.context[a].createNew(b)}):c}))}
g=e||this.dataSource||{};this.dataSource={inmemory_sequence:{}};for(var i=0;i<this.context._storageModel.length;i++){var f=this.context._storageModel[i];this.dataSource[f.TableName]=[];var j=f.LogicalType.memberDefinitions.getKeyProperties().filter(function(a){return a.computed});1<j.length&&Guard.raise(new Exception("More than one computed field not supported in "+
f.TableName+" entity set."));var k=!1;1===j.length&&(c=Container.resolveName(j[0].type),!0===this.supportedAutoincrementKeys[c]?(this.dataSource.inmemory_sequence[f.TableName]=0,k=!0):"function"!==typeof this.supportedAutoincrementKeys[c]&&console.log("WARRNING! '"+c+"' not supported as computed Key!"));if(g[f.TableName])for(c=0;c<g[f.TableName].length;c++){var h=g[f.TableName][c];h instanceof f.LogicalType||(e?h=new f.LogicalType(h):Guard.raise(new Exception("Invalid element in source: "+f.TableName)));if(k){var l=h[j[0].name];l>this.dataSource.inmemory_sequence[f.TableName]&&(this.dataSource.inmemory_sequence[f.TableName]=l)}
this.dataSource[f.TableName].push(h)}}
a.success(this.context)},executeQuery:function(a,b){var b=$data.typeSystem.createCallbackSetting(b),c;try{c=this._compile(a)}catch(e){b.error(e);return}
var d=[].concat(this.dataSource[a.context.getEntitySetFromElementType(a.defaultType).tableName]||[]);c.$filter&&!c.$every&&(d=d.filter(c.$filter));c.$map&&0===Object.keys(a.modelBinderConfig).length&&(d=d.map(c.$map));c.$order&&0<c.$order.length&&d.sort(function(a,b){for(var d,e=0,k=c.$order.length;e<k;e++){d=c.$order[e](a);var h=c.$order[e](b);d=c.$order[e].ASC?d===h?0:d>h||null===h?1:-1:d===h?0:d<h||null===d?1:-1;if(0!==d)break}
return d});void 0!==c.$take&&void 0!==c.$skip?d=d.slice(c.$skip,c.$skip+c.$take):void 0!==c.$take&&d.length>c.$take?d=d.slice(0,c.$take):c.$skip&&(d=d.slice(c.$skip,d.length));c.$some&&(d=[0<d.length]);c.$length&&(d=[d.length]);a.rawDataList=d;b.success(a)},_compile:function(a){return(new $data.storageProviders.InMemory.InMemoryCompiler(this)).compile(a)},saveChanges:function(a,b){for(var c=0;c<b.length;c++){var e=b[c];switch(e.data.entityState){case $data.EntityState.Added:this._save_add_processPk(e);this.dataSource[e.entitySet.tableName].push(e.data);break;case $data.EntityState.Deleted:var d=this.dataSource[e.entitySet.tableName],g=this._save_getEntity(e,d),e=d.indexOf(g);d.splice(e,1);break;case $data.EntityState.Modified:if(e.data.changedProperties&&0<e.data.changedProperties.length){d=this.dataSource[e.entitySet.tableName];g=this._save_getEntity(e,d);for(d=0;d<e.data.changedProperties.length;d++){var i=e.data.changedProperties[d];!i.key&&-1<e.entitySet.elementType.memberDefinitions.getPublicMappedPropertyNames().indexOf(i.name)&&(g[i.name]=e.data[i.name])}}}}
if(this.providerConfiguration.persistentData&&window.localStorage){var e=this.providerConfiguration.databaseName||"JayData_InMemory_Provider",f=this,j=[];for(c in this.context._entitySetReferences)j.push(this.context._entitySetReferences[c].collectionName);localStorageData=window.localStorage.setItem(e,JSON.stringify(this.dataSource,function(a,b){if(-1<j.indexOf(a)&&Array.isArray(b)){for(var c=[],d=0;d<b.length;d++){var e={};f.context[a].elementType.memberDefinitions.getPublicMappedProperties().forEach(function(a){if(!a.inverseProperty){var c=Container.resolveName(a.type),c=f.fieldConverter.fromDb[c];e[a.name]=c?c(b[d][a.name]):b[d][a.name]}});c.push(e)}
return c}
return b}))}
a.success()},_save_add_processPk:function(a){var b=a.entitySet.elementType.memberDefinitions.getKeyProperties();if(1===b.length&&b[0].computed){var b=b[0],c=Container.resolveName(b.type);"function"===typeof this.supportedAutoincrementKeys[c]?a.data[b.name]=this.supportedAutoincrementKeys[c]():!0===this.supportedAutoincrementKeys[c]&&(c=this.dataSource.inmemory_sequence[a.entitySet.tableName],a.data[b.name]=c+1,this.dataSource.inmemory_sequence[a.entitySet.tableName]=c+1)}else for(c=0;c<b.length;c++)(null===a.data[b[c].name]||void 0===a.data[b[c].name])&&Guard.raise(new Exception("Key field must set value! Key field name without value: "+
...
b.data+";");b.data="";b.lambda=""}},{});$C("$data.storageProviders.InMemory.InMemoryFunctionCompiler",$data.Expressions.EntityExpressionVisitor,null,{constructor:function(a){this.provider=a},compile:function(a,b){this.Visit(a,b)},VisitParametricQueryExpression:function(a,b){this.Visit(a.expression,b)},VisitUnaryExpression:function(a,b){b.data+=a.resolution.mapTo;b.data+="(";this.Visit(a.operand,b);b.data+=")"},VisitSimpleBinaryExpression:function(a,b){var c=this;if(a.resolution.reverse){if(b.data+="(","in"===a.resolution.name&&Array.isArray(a.right.value)?(b.data+="[",a.right.value.forEach(function(a,d){0<d&&(b.data+=",");c.Visit(a,b)}),b.data+="]"):this.Visit(a.right,b),b.data+=a.resolution.mapTo,this.Visit(a.left,b),a.resolution.rightValue)b.data+=a.resolution.rightValue}else b.data+="(",this.Visit(a.left,b),b.data+=a.resolution.mapTo,this.Visit(a.right,b);b.data+=")"},VisitConstantExpression:function(a,b){var c=Container.resolveType(a.type),c=this.provider.fieldConverter.escape[Container.resolveName(c)];b.data+=c?c(a.value):a.value},VisitMemberInfoExpression:function(a,b){b.data+=".";b.data+=a.memberName},VisitComplexTypeExpression:function(a,b){this.Visit(a.source,b);this.Visit(a.selector,b)},VisitEntityExpression:function(a,b){b.data+=a.selector.lambda;b.lambda=a.selector.lambda;this.Visit(a.source,b)},VisitEntitySetExpression:function(){},VisitObjectLiteralExpression:function(a,b){b.data+="{ ";for(var c=0;c<a.members.length;c++){var e=a.members[c];0<c&&(b.data+=", ");this.Visit(e,b)}
b.data+=" }"},VisitObjectFieldExpression:function(a,b){b.data+=a.fieldName+": ";this.Visit(a.expression,b)},VisitEntityFieldOperationExpression:function(a,b){Guard.requireType("expression.operation",a.operation,$data.Expressions.MemberInfoExpression);var c=a.operation.memberDefinition;c.propertyFunction&&(this.Visit(a.source,b),b.data+=".");b.data+=c.mapTo||c.name;var e=0;(c.parameters||[]).map(function(b){return b.name==="@expression"?a.source:a.parameters[e++]}).forEach(function(a,c){if(a){if(c>0)b.data=b.data+",";this.Visit(a,b)}},this);b.data+=c.rightValue||""}});