...
window.addEventListener("message",onAuthenticated);var url=local?"authorize.html":crmUrl+$data.MsCrm.Auth.clientAuthorizationPath;url=url;var w=window.open(url,"_blank","resizable=false,location=0,menubar=0,toolbar=0,width=400,height=600");}}
$data.MsCrm.init=function(crmAddress,contextType,cb){var config={};if(typeof crmAddress==='object'&&crmAddress){config=crmAddress;crmAddress=config.url;delete config.url;}
var serviceUrl=crmAddress+'/XRMServices/2011/OrganizationData.svc';if(window.location.href.indexOf(crmAddress)>-1){initContext();}else{$data.MsCrm.Auth.login(crmAddress,function(){initContext();});}
function initContext(){if(!(contextType.isAssignableTo&&contextType.isAssignableTo($data.EntityContext))){cb=contextType;config.disableBatch=$data.MsCrm.disableBatch;$data.service(serviceUrl,config,function(factory){var ctx=factory();ctx.onReady().then(function(){cb(ctx,factory);});});}else{function factory(){return new contextType({name:'oData',oDataServiceHost:serviceUrl,disableBatch:$data.MsCrm.disableBatch});}
var ctx=factory();ctx.onReady().then(function(){cb(ctx,factory);});}}}})($data);